...
function rectCollide() {
	...
	function force() {
		var node, size, mass, nodeCenterX, nodeCenterY;
		var i = -1;
		while (++i < iterations) {
			iterate();
		}
		...
		function collisionDetection(quad, x0, y0, x1, y1) {
			...
			if (data) {
				...
				var dx = nodeCenterX - xCenter(data);
				var dy = nodeCenterY - yCenter(data);
				var xDiff = Math.abs(dx) - xSize;
				var yDiff = Math.abs(dy) - ySize;
			...
			return (
				x0 > nodeCenterX + xSize ||
				y0 > nodeCenterY + ySize ||
				x1 < nodeCenterX - xSize ||
				y1 < nodeCenterY - ySize
			);
		}
